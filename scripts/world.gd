...
func _build_materials() -> void:
	...
	_mats["metal"] = _textured(_mat(Palette.METAL, 0.4, 0.7), "metal", 1.6)
	_mats["dark_metal"] = _mat(Palette.DARK_METAL, 0.45, 0.55)
	_mats["rubber"] = _textured(_mat(Palette.RUBBER, 0.95), "rubber", 3.0)
	_mats["shelf"] = _mat(Palette.SHELF, 0.8)
	_mats["stock"] = _mat(Palette.STOCK, 0.9)
	_mats["hedge"] = _textured(_mat(Palette.HEDGE, 1.0), "hedge", 2.2)
